...
By default, in `vaults/` inside the directory you run the command from — never
beside the code. From a clone, `bun run decant` always runs in the repository
root, whichever subdirectory you call it from, so that is where they land.

To keep them in one place instead, point `DECANT_VAULTS` at an absolute path:

```bash
export DECANT_VAULTS=~/Documents/decant     # bash, zsh
set -Ux DECANT_VAULTS ~/Documents/decant    # fish
```

The vaults then land there wherever you run the command. Inside a clone this
...
- the path has to be absolute; a leading `~` is expanded even when the shell
  left it quoted
- only the last directory is created — when its parent does not exist, the run
  stops and says so
- a run that created the directory and then failed removes it again, as long as
